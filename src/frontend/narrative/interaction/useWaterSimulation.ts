    ...
    for (let dy = -radius; dy <= radius; dy++) {
      for (let dx = -radius; dx <= radius; dx++) {
        ...
        if (x >= 1 && x < SIM_COLS - 1 && y >= 1 && y < SIM_ROWS - 1) {
          ...
          if (dist <= radius) {
            buf[y * SIM_COLS + x] += (1 - dist / radius) * strength * 255;
          }
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      if (flowState === 1 && time - lastAutoDrop > 800) {
        lastAutoDrop = time;
        dropRipple(0.3 + Math.sin(time * 0.001) * 0.2, 0.4 + Math.cos(time * 0.0013) * 0.2, 0.45, 2);
      } else if (flowState === 2 && time - lastAutoDrop > 350) {
        lastAutoDrop = time;
        dropRipple(0.5 + (Math.random() - 0.5) * 0.6, 0.5 + (Math.random() - 0.5) * 0.6, 0.8, 3);
      }
      ...
      if (imgData && offCtx) {
        ...
        for (let y = 1; y < SIM_ROWS - 1; y++) {
          ...
          for (let x = 1; x < SIM_COLS - 1; x++) {
            ...
            data[pIdx] = Math.min(255, Math.max(0, 40 + intensity * 0.9));
            data[pIdx + 1] = Math.min(255, Math.max(0, 58 + intensity * 1.1));
            data[pIdx + 2] = Math.min(255, Math.max(0, 70 + intensity * 1.3));
            data[pIdx + 3] = Math.min(255, Math.max(0, Math.abs(intensity) * 2.2 + 20));
          }
        }
